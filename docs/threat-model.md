...
- Admins befolgen das [Release-Runbook](https://github.com/tobiasnix/anlaufstelle/issues/502) und die [Release-Checkliste](release-checklist.md)
...
| **E** Privilege-Escalation via Form-Tampering | Server-seitige Permission-Checks via Mixin-Hierarchie + `can_user_mutate_*`-Helper | [`mixins.py`](../src/core/views/mixins.py), [`workitem_actions.py`](../src/core/views/workitem_actions.py) | — (WorkItem-Race [#129](https://github.com/tobiasnix/anlaufstelle/issues/129) Teil A geschlossen) |

### TB2/TB3 — Django ↔ PostgreSQL

| Threat | Mitigation (Bestand) | Quelle | Offene Lücke |
|---|---|---|---|
| **S** Connection-Pool-Tenant-Leak | `FacilityScopeMiddleware` setzt `app.current_facility_id` pro Request — auch leer für anonyme Requests | [`facility_scope.py`](../src/core/middleware/facility_scope.py) — Tier-2 Refs [#733](https://github.com/tobiasnix/anlaufstelle/issues/733) | — |
...
| **D** ClamAV-Outage blockt Uploads | Fail-closed gewollt — Risiko: Service-Verfügbarkeit. Healthcheck flag in `/health/` | Refs [#524](https://github.com/tobiasnix/anlaufstelle/issues/524) | Offene Frage Healthcheck differenziert (`503` bei ClamAV-Ausfall) — Master-Audit Maßnahme #24 |

### TB5 — Off-Site-Backups

| Threat | Mitigation (Bestand) | Quelle | Offene Lücke |
|---|---|---|---|
| **C** Backup-Diebstahl | AES-256-CBC + PBKDF2 mit `BACKUP_ENCRYPTION_KEY`; DB + Medien gemeinsam ([Tier-3 #720](https://github.com/tobiasnix/anlaufstelle/issues/720)) | [`backup.sh`](../scripts/backup.sh) | Off-Site-Hook (rclone/restic/S3) fehlt — Master-Audit Maßnahme #22 |
...
- Aktive Tracking-Issues: [#681](https://github.com/tobiasnix/anlaufstelle/issues/681), [#691](https://github.com/tobiasnix/anlaufstelle/issues/691), [#684](https://github.com/tobiasnix/anlaufstelle/issues/684), [#695](https://github.com/tobiasnix/anlaufstelle/issues/695)
...
Letzte Revision: **2026-04-29** (initiale Fassung im Rahmen Tier-3-Sprint, Refs [#691](https://github.com/tobiasnix/anlaufstelle/issues/691), [#733](https://github.com/tobiasnix/anlaufstelle/issues/733)).
