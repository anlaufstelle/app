...
| Begriff | Definition |
|---------|------------|
| **Account-Lockout** | Automatische Konto-Sperre nach 10 fehlgeschlagenen Login-Versuchen (seit v0.10.1). Der Sperrzustand wird nicht separat gespeichert, sondern aus den `LOGIN_FAILED`-AuditLog-Einträgen im gleitenden 15-Minuten-Fenster abgeleitet; Admins entsperren betroffene Nutzer über die Profilansicht, was einen `LOGIN_UNLOCK`-Eintrag schreibt (frühere `LOGIN_FAILED`-Einträge zählen danach nicht mehr mit). Implementierung: [`src/core/services/security/login_lockout.py`](https://github.com/anlaufstelle/app/blob/main/src/core/services/security/login_lockout.py). |
...
| **File Vault** | Datei-Anhänge werden verschlüsselt im Ruhezustand gespeichert (Fernet/AES-128). Dateien werden vor der Verschlüsselung von ClamAV gescannt. Downloads laufen über einen zentralen Safe-Download-Helfer mit RFC-5987-Content-Disposition. |
...
| **Kontaktstufe** | Der Identifizierungsgrad einer Person im System. Das Datenmodell kennt zwei Kontaktstufen (`Client.ContactStage`): identifiziert (Pseudonym bekannt) und qualifiziert (zusätzliche persönliche Daten, eingeschränkter Zugriff). Bestimmt Zugriffsrechte, zulässige Dokumentationstypen und Löschfristen. Ein rein anonymer Kontakt (nur Zählung) ist keine Kontaktstufe, sondern ein personenloses Ereignis (`Event.is_anonymous`). |
...
| **Offline-Modus (M6A)** | Client-seitig verschlüsselte Offline-Erfassung für Streetwork. Nutzt AES-GCM-256 mit einem PBKDF2-abgeleiteten (600 000 Iterationen, SHA-256) non-extractable `CryptoKey` in einer eigenen IndexedDB (`anlaufstelle-crypto`); die rohen Schlüssel-Bytes sind nie exportierbar. Wird bei Logout, Passwortwechsel, Rollenentzug/Deaktivierung (server-seitige Salt-Rotation) und bei Session-Leerlauf gelöscht (Idle-Key-Wipe), nicht beim Schließen des Tabs. Lesen, Schreib-Queue **und** In-App-Offline-**Edit** (inkl. Konflikt-/Dead-Letter-Ansicht unter `/offline/conflicts/`) sind der akzeptierte Produktiv-Umfang — der Edit-Einstieg ist seit #1111 verdrahtet und erprobt (nicht mehr deferred). Vertiefung unten. Siehe [ADR-022](adr/022-offline-snapshot-keys.md). |
