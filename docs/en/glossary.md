...
| Offline-Modus (M6A) | Offline mode (M6A) | Client-side encrypted offline capture for streetwork. Uses AES-GCM-256 with a PBKDF2-derived (600 000 iterations, SHA-256) non-extractable `CryptoKey` held in a dedicated IndexedDB (`anlaufstelle-crypto`); the raw key bytes can never be exported. Wiped on logout and on session-idle (idle-key-wipe), not on tab close. Read + write-queue are the accepted production scope; the in-app offline-**edit** entry is deferred (#1111). See [ADR-022](../adr/022-offline-snapshot-keys.md). |
