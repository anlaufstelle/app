...
class TestBulkManualSelectionUI:
    ...
    def test_manual_checkbox_works_after_person_filter_swap(self, staff_page, base_url):
        ...
        _create_workitem_assigned(page, base_url, f"E2E-Swap-{tag}-1", assignee_label="Lena Weber")
        _create_workitem_assigned(page, base_url, f"E2E-Swap-{tag}-2", assignee_label="Lena Weber")
    ...
    def test_select_all_then_deselect_after_person_filter(self, staff_page, base_url):
        ...
        page = staff_page
        tag = uuid.uuid4().hex[:6]
        _create_workitem_assigned(page, base_url, f"E2E-PFDesel-{tag}-1", assignee_label="Lena Weber")
        _create_workitem_assigned(page, base_url, f"E2E-PFDesel-{tag}-2", assignee_label="Lena Weber")
        _create_workitem_assigned(page, base_url, f"E2E-PFDesel-{tag}-3", assignee_label="Lena Weber")
...
class TestBulkPreservesFilterUI:
    ...
    def test_person_filter_preserved_in_url_after_bulk_apply(self, staff_page, base_url):
        ...
        page = staff_page
        tag = uuid.uuid4().hex[:6]
        _create_workitem_assigned(page, base_url, f"E2E-PFKeep-{tag}", assignee_label="Lena Weber")
...
def _create_workitem_assigned(page, base_url, title: str, assignee_label: str | None) -> None:
    ...
    page.click("button:has-text('Speichern')")
    # Erfolgreiches Anlegen == Redirect auf die Liste (bei Validierungsfehler
    # bliebe man auf /new/). Den Link in der Default-Liste NICHT abwarten: einer
    # ANDEREN Person zugewiesene Items erscheinen dort nicht — nur unter dem
    # Personenfilter (genau das pruefen die Tests). Die UUID braucht kein Aufrufer.
    page.wait_for_url(re.compile(r"/workitems/$"), timeout=10000)
