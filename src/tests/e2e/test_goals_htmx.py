...
def _create_goal_in_case(page, title: str) -> None:
    ...
    page.locator(f"#goals-section :text-is('{title}')").first.wait_for(state="visible", timeout=5000)


def _goal_card_locator(page, title: str):
    """Container der Goal-Card mit dem gegebenen Title."""
    return page.locator("#goals-section > div > div").filter(has=page.locator(f"h3:has-text('{title}')"))
...
class TestGoalEditViaHTMX:
    ...
    def test_edit_goal_title_and_description_persists(self, staff_page, base_url):
        ...
        edit_button = card_heading.locator("xpath=following-sibling::button[1]")
        edit_button.click()
        ...
        page.locator(f"#goals-section h3:text-is('{new_title}')").wait_for(state="visible", timeout=5000)
...
class TestGoalToggleViaHTMX:
    ...
    def test_toggle_goal_to_achieved_and_back(self, staff_page, base_url):
        ...
        page.locator(f"#goals-section h3:has-text('{title}')").first.wait_for(state="visible", timeout=5000)
        ...
        toggle_back = card_after.locator("form[hx-post$='/toggle/'] button[type='submit']").first
        toggle_back.click()
    ...
    def test_three_goals_two_milestones_each_render_after_reload(self, staff_page, base_url):
        ...
        for gt in goal_titles:
            ...
            for i in range(2):
                ...
                page.locator(f"#goals-section :text-is('{ms_t}')").first.wait_for(state="visible", timeout=5000)
                ms_titles[gt].append(ms_t)

        # Bei Goal „Wohnung" (erstes): ersten Meilenstein toggeln.
        wohnung = goal_titles[0]
        _goal_card_locator(page, wohnung)
        ...
        page.locator(f"#goals-section span.line-through:has-text('{first_ms}')").wait_for(state="visible", timeout=5000)
        ...
        page.locator(f"#goals-section h3:text-is('{job}')").wait_for(state="visible", timeout=5000)
        # Erreicht-Badge in der Job-Card.
        _goal_card_locator(page, job).locator(":text-is('erreicht')").wait_for(state="visible", timeout=5000)
